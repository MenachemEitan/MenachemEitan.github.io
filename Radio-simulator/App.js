...
export default function App() { 
  const [url, setUrl] = useState('https://menachemeitan.github.io/'); 
  ...
  registerNNPushToken(10146, 'UQ7gHOgfCDUjN8EjTaCXbf'); 
  ...
  useEffect(() => { 
    async function getUnreadNots() { 
      let unreadCount = await getUnreadNotificationInboxCount(10146, 'UQ7gHOgfCDUjN8EjTaCXbf'); 
  ...
  const handleAnalytics = (path) => { 
    axios.post(`https://app.nativenotify.com/api/analytics`, { 
      app_id: 10146, 
      app_token: 'UQ7gHOgfCDUjN8EjTaCXbf', 
      screenName: path 
    }); 
  }; 
  
  const handleGoToInbox = async () => { 
    let notifications = await getNotificationInbox(10146, 'UQ7gHOgfCDUjN8EjTaCXbf'); 
